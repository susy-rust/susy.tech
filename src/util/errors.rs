...
/// (see <https://github.com/susy-rust/cargo/issues/3995>), but Ember requires
...
impl AppError for ReadOnlyMode {
    ...
    fn response(&self) -> Option<Response> {
        let mut response = json_response(&Bad {
            errors: vec![StringError {
                detail: "Susy.tech is currently in read-only mode for maintenance. \
            ...
            errors: vec![StringError {
                detail: format!(
                    "You have published too many crates in a \
                     ...
                     help@susy.tech to have your limit increased.",
                    retry_after
                ),
            }],
        });
