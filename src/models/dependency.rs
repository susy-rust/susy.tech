        ...
        .map(|dep| {
            if let Some(registry) = &dep.registry {
                if !registry.is_empty() {
                    return Err(cargo_err(&format_args!("Dependency `{}` is hosted on another registry. Cross-registry dependencies are not permitted on susy.tech.", &*dep.name)));
                }
            }
            ...
            if dep.version_req == semver::VersionReq::parse("*").unwrap() {
                return Err(cargo_err(
                    "wildcard (`*`) dependency constraints are not allowed \
                     on susy.tech. See https://doc.rust-lang.org/cargo/faq.html#can-\
