...
fn new_krate_with_dependency() {
    ...
    app.db(|conn| {
        ...
        // https://github.com/susy-rust/susy.tech/issues/651
...
fn reject_new_crate_with_alternative_registry_dependency() {
    ...
    assert!(
        json.errors[0]
            .detail
            .contains("Cross-registry dependencies are not permitted on susy.tech."),
        "{:?}",
        json.errors
    );
}
...
fn new_krate_without_any_email_fails() {
    ...
    assert!(
        json.errors[0]
            .detail
            .contains("A verified email address is required to publish crates to susy.tech"),
        "{:?}",
        json.errors
    );
}
...
fn good_badges() {
    ...
    badge_attributes.insert(
        ...
        String::from("susy-rust/susy.tech"),
    );
    ...
    assert_eq!(
        ...
        Some(String::from("susy-rust/susy.tech"))
    );
}
...
fn ignored_badges() {
    ...
    unknown_badge_attributes.insert(String::from("repository"), String::from("susy-rust/rust"));
