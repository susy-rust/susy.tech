...
impl crate::util::MockCookieUser {
    ...
    fn update_email_more_control(&self, user_id: i32, email: Option<&str>) -> Response<OkBool> {
        // When updating your email in susy.tech, the request goes to the user route with PUT.
...
fn show_latest_user_case_insensitively() {
    ...
    app.db(|conn| {
        ...
        // with the same gh_login in susy.tech. `gh_id` is stable across renames, so that field
        // should be used for uniquely identifying GitHub accounts whenever possible. For the
        // susy.tech/user/:username pages, the best we can do is show the last susy.tech account
...
/*  Given a GitHub user, check that if the user logs in,
    ...
    email they added to susy.tech will not be overwritten
...
fn github_without_email_does_not_overwrite_email() {
    ...
    // Add an email address in susy.tech
...
/* Given a new user, test that if they sign in with one email, change their email on GitHub, then
   sign in again, that the email in susy.tech will remain set to the original email used on GitHub.
*/
#[test]
fn github_with_email_does_not_overwrite_email() {
    ...
    // Simulate logging in to susy.tech after changing your email in GitHub
...
/*  Given a susy.tech user, check that the user's email can be
...
/*  Given a susy.tech user, check to make sure that the user
