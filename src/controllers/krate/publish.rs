...
/// --status` command, via susy.tech's front end, or email.
pub fn publish(req: &mut dyn Request) -> AppResult<Response> {
    ...
    let verified_email_address = verified_email_address.ok_or_else(|| {
        cargo_err(
            "A verified email address is required to publish crates to susy.tech. \
             Visit https://susy.tech/me to set and verify your email address.",
        )
    })?;
    ...
    conn.transaction(|| {
        ...
        // that is no longer needed. As such, susy.tech currently does not return any `other`
